...
class Window(tk.Tk):
    ...
    def setup_map(self, parent):
        ...
        self.pie_chart_button = ttk.Button(parent, text="顯示圓餅圖", command=self.show_pie_chart)
    ...
    def show_pie_chart(self):
        ...
        # Read CSV and filter data based on selected criteria
        data = []
        with open('112年度A2交通事故資料_1.csv', newline='', encoding='utf-8') as csvfile:
            reader = csv.DictReader(csvfile)
            for row in reader:
                if (row['地區'] in selected_cities and
                    row['天氣'] in selected_weather and
                    row['光線狀態'] in selected_light and
                    row['肇事逃逸'] in selected_run):
                    data.append(row)
    ...
    def show_pie_chart(self):
        ...
        #  dictionary ---> pandas DataFrame for easier plotting
        df = pd.DataFrame(list(accident_types.items()), columns=['事故類別', 'Count'])

        # pie chart
        plt.figure(figsize=(8, 6))
        plt.pie(df['Count'], labels=df['事故類別'], autopct='%1.1f%%', startangle=140)
        plt.title('事故類別分佈')
