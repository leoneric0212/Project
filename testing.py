...
class Window(tk.Tk):
    ...
    def submit_data(self):
        ...
        try:
            ...
            messagebox.showerror("File Not Found", f"找不到 {selected_year}.csv 資料")
            return

        # Convert '發生日期' to string format to facilitate filtering
        df['發生日期'] = pd.to_datetime(df['發生日期'], format='%Y-%m-%d %H:%M:%S').dt.date.astype(str)

        # Filter based on selected year, month, and day
        filtered_df = df[
            (df['發生日期'].str.startswith(f"{selected_year}-{selected_month.zfill(2)}-{selected_day.zfill(2)}")) &
    ...
    def populate_respond(self,data):
        ...
        # Format time to remove decimals
